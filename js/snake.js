...
function setMoveEvents() {
    ...
    $('body').on('keydown', function (e) {
        //Arrow keycodes
        if (e.which === 65 && lastDirectionString !== 'right') {
            directionString = 'left';
        } else if (e.which === 87 && lastDirectionString !== 'down') {
            directionString = 'up';
        } else if (e.which === 68 && lastDirectionString !== 'left') {
            directionString = 'right';
        } else if (e.which === 83 && lastDirectionString !== 'up') {
    ...
    $elem.on('keydown', function (e) {
        if (e.which === 65 && lastDirectionString === 'left'
            || e.which === 87 && lastDirectionString === 'up'
            || e.which === 68 && lastDirectionString === 'right'
            || e.which === 83 && lastDirectionString === 'down') {
...
function checkNewPosition() {
    ...
    for (var z = 1; z < snake.length; z++) {
        ...
        if (head.position().top === part.position().top && head.position().left === part.position().left) {
            showInfoAlert('<h4>Game over!</h4><br/>Pontuação: ' + snake.length + '<br/><br/><a href="#" onclick="location.reload();" onkeydown="javascript: if(event.keyCode == 13) location.reload();">Iniciar novo jogo!</a>');
